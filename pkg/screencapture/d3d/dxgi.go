...
func NewIDXGIOutputDuplication(device *ID3D11Device, deviceCtx *ID3D11DeviceContext, output uint) (*OutputDuplicator, error) {
	...
	return &OutputDuplicator{device: device, deviceCtx: deviceCtx, outputDuplication: dup, needsSwizzle: needsSwizzle, rect: rect, rotation: DXGI_MODE_ROTATION(desc.Rotation)}, nil
}
...
func (obj *IDXGIOutput) GetDesc(desc *_DXGI_OUTPUT_DESC) int32 {
	ret, _, _ := syscall.Syscall(
		obj.vtbl.GetDesc,
		2,
		uintptr(unsafe.Pointer(obj)),
...
func (obj *IDXGIOutputDuplication) GetOutputDesc(desc *_DXGI_OUTPUT_DESC) int32 {
	ret, _, _ := syscall.Syscall(
		obj.vtbl.GetDesc,
		2,
		uintptr(unsafe.Pointer(obj)),
